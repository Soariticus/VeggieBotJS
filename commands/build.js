...
module.exports = {
	...
	async execute(interaction) {
		...
		var processedThrowable = { name: "Throwable", value: ranThrowable['name'], inline:true }; // We turn throwable into a dictionary in advance

		var fieldArray = [ // All of the dictionaries get slapped into an array
		...
		for(var i = 0; i < perkWithThrowableArray.length; i++){
			if(ranPerk['name'] == perkWithThrowableArray[i]){ // If the perkdeck has a unique throwable, we make the uniqueThrow var true
				uniqueThrow = true;
			}
		}
		if(uniqueThrow == false){ // Only if the perk deck doesn't have a unique throwable do we add it to the array
